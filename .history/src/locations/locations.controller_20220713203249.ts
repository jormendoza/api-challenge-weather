...
export class LocationsController {
    ...
    @ApiOperation({ summary: 'Obtiene una lista de regines geograficas disponibles' })
    @Get('regions')
    getRegions() {
        return this.locationsService.findAllRegions();
    }

    @ApiOperation({ summary: 'Obtiene una lista de paises de una region geográfica que se recibe por parametro' })
    @Get('countries/:idRegion')
    getCountries(@Param('idRegion') idRegion: string) {
        return this.locationsService.findCountries(idRegion);
    }

    @ApiOperation({ summary: 'Obtiene una lista de ciudades que coinciden con una búsqueda' })
    ...
    @ApiOperation({ summary: 'Obtiene una lista de provincias de una país que se recibe por parametro' })
